...
fs.readdirSync(`../build/images`).forEach((file) => {
  const formData = new FormData();
  const fileStream = fs.createReadStream(`../build/images/${file}`);
  ...
  fetch(url, options)
    ...
    .then((json) => {
      ...
      fs.writeFileSync(
        `../build/meta/_metadata.json`, // write generated ipfs_url to metadata json file
        JSON.stringify(meta_data, null, 2)
      );
      console.log(`${json.file_name} uploaded & _metadata.json updated`);
    })
